...
export async function seedIsoline(pool: Pool) {
  try {
    const res = await pool.query('SELECT id FROM "Product" WHERE slug = $1 LIMIT 1', ['horizontalni-zaluzie-isoline']);
    if (res.rows.length > 0) {
      return; // already seeded
    }

    console.log("Seeding Horizontální žaluzie Isoline...");
    ...
    const parameters = [
      {
        ...
        "type": "select",
        "options": [
          { "label": "25 x 0,18 mm (základní)", "value": "25_018", "priceVariant": 0, "priceType": "fixed" },
          { "label": "25 x 0,21 mm", "value": "25_021", "priceVariant": 74, "priceType": "per_m2" },
          { "label": "16 x 0,21 mm", "value": "16_021", "priceVariant": 74, "priceType": "per_m2" },
          { "label": "Perforované lamely", "value": "perfo", "priceVariant": 76, "priceType": "per_m2" }
        ],
        "required": true
      },
      {
        ...
        "type": "select",
        "options": [
          { "label": "Základní odstín", "value": "zakladni", "priceVariant": 0, "priceType": "fixed" },
          { "label": "Příplatkový odstín", "value": "priplatek", "priceVariant": 87, "priceType": "per_m2" },
      ...
      },
      {
        ...
        "type": "select",
        "options": [
          { "label": "Základní", "value": "zakladni", "priceVariant": 0, "priceType": "fixed" },
          { "label": "Příplatková (RAL)", "value": "ral", "priceVariant": 147, "priceType": "per_bm" },
          { "label": "Příplatková (Imitace dřeva)", "value": "drevo", "priceVariant": 131, "priceType": "per_bm" }
        ],
        "required": true
      },
      {
        ...
        "type": "select",
        "options": [
          { "label": "Ne (Standard)", "value": "ne", "priceVariant": 0, "priceType": "fixed" },
          { "label": "Ano (Domykatelné)", "value": "ano", "priceVariant": 33, "priceType": "per_m2" }
        ],
        "required": true
      },
      {
        ...
        "type": "select",
        "options": [
          { "label": "Vpravo (ŘP)", "value": "vpravo", "priceVariant": 0, "priceType": "fixed" },
          { "label": "Vlevo (ŘL)", "value": "vlevo", "priceVariant": 0, "priceType": "fixed" }
        ],
        "required": true
      }
    ];
    ...
    const prodRes = await pool.query(`
      INSERT INTO "Product" (
        title, slug, category, "desc", img, price, hidden,
    ...
    `, [
      'Horizontální žaluzie Isoline', 'horizontalni-zaluzie-isoline', category, 
      '<p>Základní ceníková sestava s horním a dolním profilem z válcovaného pozinkovaného plechu a hliníkovou lamelou.</p>', 
      '/zaluzie_isoline.png', 263, false,
      200, 2400, 300, 2500, 2.4,
      'matrix_cell', JSON.stringify(parameters), '[]'
    ]);
...
2200 483 537 597 652 737 795 852 946 1003 1058 1118 1238 1293 1350 1417 1462 1514 1639 1733 1862
    ...
    console.log("Isoline successfully seeded.");
  } catch (e) {
    console.error("Failed to seed isoline", e);
  }
}
